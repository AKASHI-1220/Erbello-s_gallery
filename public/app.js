(() => {
  ...
  function initialRoute() {
    ...
    if (path === 'projects' || path === 'about' || path === 'contact' || path === 'privacy' || path === 'terms') return path;
    return 'home';
  }
  ...
  const POLICY_PAGES = {
    ko: {
      privacy:{ eyebrow:'PRIVACY', title:'개인정보처리방침', body:'ERBELLO는 프로젝트 갤러리 운영과 기본적인 사이트 이용을 위해 필요한 최소한의 정보만 사용합니다.', blocks:[{title:'수집하는 정보', text:'방문 기록, 조회수, 문의 링크 이용과 같이 사이트 운영에 필요한 기본 정보가 사용될 수 있습니다.'},{title:'광고와 쿠키', text:'Google AdSense가 광고 제공 및 통계 목적으로 쿠키를 사용할 수 있습니다.'},{title:'문의', text:'개인정보 관련 문의는 연락처 페이지의 링크를 통해 보낼 수 있습니다.'}] },
      terms:{ eyebrow:'TERMS', title:'이용약관', body:'ERBELLO 프로젝트 갤러리 이용에 관한 기본 안내입니다.', blocks:[{title:'개인 프로젝트 갤러리', text:'ERBELLO는 HTML, JSX, ZIP 기반의 개인 프로젝트를 정리하고 공유하기 위한 갤러리입니다.'},{title:'콘텐츠 이용', text:'등록된 프로젝트의 저작권과 책임은 각 프로젝트 작성자에게 있으며, 무단 복제나 재배포는 권장하지 않습니다.'},{title:'서비스 변경', text:'사이트 구조, 프로젝트, 링크, 기능은 운영 상황에 따라 수정되거나 삭제될 수 있습니다.'}] }
    },
    en: {
      privacy:{ eyebrow:'PRIVACY', title:'Privacy Policy', body:'ERBELLO uses only the minimum information needed to run the project gallery and basic site features.', blocks:[{title:'Information used', text:'Basic operational data such as visit activity, view counts, and contact link usage may be used.'},{title:'Ads and cookies', text:'Google AdSense may use cookies for ad delivery and measurement.'},{title:'Contact', text:'For privacy questions, use the links on the contact page.'}] },
      terms:{ eyebrow:'TERMS', title:'Terms', body:'Basic terms for using the ERBELLO project gallery.', blocks:[{title:'Personal project gallery', text:'ERBELLO is a gallery for organizing and sharing HTML, JSX and ZIP-based personal projects.'},{title:'Content use', text:'Project rights and responsibilities remain with each project author. Unauthorized copying or redistribution is not recommended.'},{title:'Service changes', text:'Site structure, projects, links and features may be changed or removed depending on operation needs.'}] }
    },
    ja: {
      privacy:{ eyebrow:'PRIVACY', title:'プライバシーポリシー', body:'ERBELLOはプロジェクトギャラリー運営と基本機能のために必要最小限の情報を使用します。', blocks:[{title:'使用する情報', text:'訪問記録、閲覧数、連絡先リンク利用など、運営に必要な基本情報が使われる場合があります。'},{title:'広告とCookie', text:'Google AdSenseが広告配信と測定のためにCookieを使用する場合があります。'},{title:'お問い合わせ', text:'個人情報に関するお問い合わせは連絡先ページのリンクをご利用ください。'}] },
      terms:{ eyebrow:'TERMS', title:'利用規約', body:'ERBELLOプロジェクトギャラリー利用に関する基本案内です。', blocks:[{title:'個人プロジェクトギャラリー', text:'ERBELLOはHTML、JSX、ZIPベースの個人プロジェクトを整理し共有するためのギャラリーです。'},{title:'コンテンツ利用', text:'登録されたプロジェクトの権利と責任は各作成者にあります。無断複製や再配布は推奨しません。'},{title:'サービス変更', text:'サイト構造、プロジェクト、リンク、機能は運営状況により変更または削除される場合があります。'}] }
    },
    zh: {
      privacy:{ eyebrow:'PRIVACY', title:'隐私政策', body:'ERBELLO 仅使用运行项目画廊和基本网站功能所需的最少信息。', blocks:[{title:'使用的信息', text:'可能使用访问记录、浏览量、联系链接使用等运营所需的基本信息。'},{title:'广告和 Cookie', text:'Google AdSense 可能会使用 Cookie 用于广告投放和统计。'},{title:'联系', text:'有关隐私的问题，可通过联系页面的链接发送。'}] },
      terms:{ eyebrow:'TERMS', title:'使用条款', body:'ERBELLO 项目画廊使用相关的基本说明。', blocks:[{title:'个人项目画廊', text:'ERBELLO 用于整理和分享基于 HTML、JSX、ZIP 的个人项目。'},{title:'内容使用', text:'已登记项目的权利与责任属于各项目作者，不建议未经许可复制或再分发。'},{title:'服务变更', text:'网站结构、项目、链接和功能可能根据运营情况修改或删除。'}] }
    }
  };
  ...
  function pageContent(slug = currentRoute, lang = currentLang) {
    ...
    const base = clone(defaults[slug] || policyDefaultPage(slug, lang) || fallbackDefaults[slug] || policyDefaultPage(slug, 'ko') || {});
