...
const AI_POST_BODY_MIN_CHARS = { diary: 260, trend: 700 };
...
function normalizeAiPostingConfig(input = {}) {
  ...
  const maxOutputTokens = !Number.isFinite(rawMaxTokens) || rawMaxTokens === 900 || rawMaxTokens === 1400
  ...
  return {
    ...
    maxOutputTokens: Math.round(cleanNumber(maxOutputTokens, DEFAULT_AI_POSTING_CONFIG.maxOutputTokens, 300, 2400)),
...
function aiPostBodyMinChars(kind) {
  return AI_POST_BODY_MIN_CHARS[kind === 'trend' ? 'trend' : 'diary'];
}

function aiPostBodyLooksShort(kind, body) {
  return bodyPlainText(body).length < aiPostBodyMinChars(kind);
}
...
function decorateAiPostBody(kind, generated, config) {
  ...
  if (aiPostBodyLooksShort(safeKind, body)) return body;

  const assets = AI_POST_ASSETS[safeKind] || AI_POST_ASSETS.diary;
  const seed = `${safeKind}|${generated && generated.title || ''}|${body.slice(0, 80)}`;
  const decorationPool = safeKind === 'trend'
    ? assets.divider
    : [...(assets.divider || []), ...(assets.sticker || [])];
  const decoration = assetMarkdown(
    pickAsset(decorationPool, `${seed}|single-decoration`),
    safeKind === 'trend' ? 'trend divider' : 'diary asset'
  );
  return [body, decoration].filter(Boolean).join('\n\n');
}
...
function buildGeminiPrompt(kind, config, recentPosts = []) {
  ...
  const completenessRule = safeKind === 'trend'
    ? '- Write a complete, readable trend post. Do not return a one-line summary.'
    : '- Write a complete short diary. Do not return a one-line summary.';
  return [
    ...
    lengthRule,
    '- The body must contain original writing, not only a title, summary, or decorative wording.',
...
function buildGeminiRetryPrompt(kind, config, recentPosts = [], previousPost = {}) {
  const safeKind = kind === 'trend' ? 'trend' : 'diary';
  const previousBodyLength = bodyPlainText(previousPost.body).length;
  const detail = safeKind === 'trend'
    ? '이번에는 본문을 8~12개의 짧은 문단으로 충분히 작성하세요. 개인 사이트, 블로그, AI 도구 활용 흐름을 관찰 중심으로 비교하고, 확인되지 않은 최신 뉴스처럼 단정하지 마세요.'
    : '이번에는 본문을 8~9줄 정도의 짧은 일기 문단으로 충분히 작성하세요. 펠로가 갤러리를 둘러본 느낌, 작은 발견, 방문자에게 건네는 인사를 자연스럽게 넣어주세요.';
  return [
    buildGeminiPrompt(safeKind, config, recentPosts),
    '',
    'The previous output was rejected because the body was too short.',
    `Previous title: ${cleanString(previousPost.title, 80) || '(none)'}`,
    `Previous body length: ${previousBodyLength}`,
    detail,
    'Retry now. Return only valid JSON with a complete body.'
  ].join('\n');
}
