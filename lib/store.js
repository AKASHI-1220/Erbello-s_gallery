...
const TAROT_RETENTION_MS = 30 * 24 * 60 * 60 * 1000;
...
function normalizeArtifact(item) {
  ...
  return {
    ...
    show_gallery_return: row.show_gallery_return !== false,
...
const LIST_COLUMNS = 'id, title, description, type, tags, source_kind, source_filename, status, is_jsx, created_at, updated_at, view_count, cover_image, gallery_images, detail_text, is_private, show_gallery_return';
  ...
  if (!supabase) {
    try {
      ...
      return {
        ok:true,
        mode,
        artifactCount:Array.isArray(items) ? items.length : 0
      };
    } catch (error) {
      return {
        ok:false,
...
function recordFromPayload(payload, existing = {}) {
  return normalizeArtifact({
    ...
    show_gallery_return: payload.show_gallery_return !== false,
...
function dbPayload(record, forUpdate = false) {
  const data = {
    ...
    show_gallery_return: record.show_gallery_return !== false,
...
function normalizeStoredTarotCode(value) {
...
function normalizeTarotQuestionContext(value) {
  const src = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const readingType = normalizeReadingTypeId(src.readingType || src.reading_type);
  const fieldsSource = src.fields && typeof src.fields === 'object' && !Array.isArray(src.fields) ? src.fields : {};
  const fields = {};
  Object.entries(fieldsSource).forEach(([key, raw]) => {
    const cleanKey = String(key || '').replace(/[^a-zA-Z0-9_]/g, '').slice(0, 40);
    const cleanValue = String(raw || '').trim().replace(/\s+/g, ' ').slice(0, 260);
    if (cleanKey && cleanValue) fields[cleanKey] = cleanValue;
  });
  const label = String(src.readingTypeLabel || src.reading_type_label || TAROT_READING_TYPE_LABELS[readingType] || '').trim().slice(0, 40) || TAROT_READING_TYPE_LABELS[readingType];
  const subTopic = String(src.subTopic || src.sub_topic || '').trim().slice(0, 24);
  return {
    readingType,
...
function tarotQuestionContextFallback(question, context) {
  const ctx = normalizeTarotQuestionContext(context);
  const rows = [[ '리딩 종류', ctx.readingTypeLabel ]];
  if (ctx.subTopic) rows.push(['세부 주제', ctx.subTopic]);
  Object.values(ctx.fields || {}).forEach(value => rows.push(['정보', value]));
  const prefix = rows.map(([label, value]) => `${label}: ${value}`).join('\n');
  return `${prefix}\n\n질문: ${String(question || '').trim()}`.trim().slice(0, 400);
}
...
function normalizeTarotSettings(value) {
  ...
  const adminCode = normalizeStoredTarotCode(get('adminCode', 'admin_code', ''));
...
function normalizeTarotSubmission(row) {
  ...
  return {
    ...
    questionContext:normalizeTarotQuestionContext(item.question_context || item.questionContext),
    question_context:normalizeTarotQuestionContext(item.question_context || item.questionContext),
    spreadCount:Number(item.spread_count || item.spreadCount || 3),
    spreadType:String(item.spread_type || item.spreadType || ''),
...
function tarotSubmissionDbPayload(payload, options = {}) {
  const item = normalizeTarotSubmission(payload);
  const includeQuestionContext = options.includeQuestionContext !== false;
  ...
  if (includeQuestionContext) row.question_context = item.questionContext;
  return row;
}
...
function tarotInviteCleanupDue(invite, activeInviteIds, now = Date.now()) {
  const id = String(invite && invite.id || '');
  if (!id || activeInviteIds.has(id)) return false;
  const createdAt = msFromDate(invite.created_at || invite.createdAt);
  const expiresAt = msFromDate(invite.expires_at || invite.expiresAt);
  return Boolean(expiresAt && expiresAt <= now)
    || Boolean(createdAt && createdAt + TAROT_RETENTION_MS <= now);
}

async function deleteExpiredTarotData() {
  if (supabase) {
    const rpcResult = await supabase.rpc('tarot_delete_expired_data');
    ...
    if (submissionDelete.error) {
      if (isMissingColumnOrTable(submissionDelete.error)) return false;
      throw submissionDelete.error;
    }

    const [inviteResult, activeSubmissionResult] = await Promise.all([
      supabase.from('tarot_invites').select('id, status, created_at, expires_at'),
      supabase.from('tarot_submissions').select('invite_id')
    ]);
    if (inviteResult.error) throw inviteResult.error;
    if (activeSubmissionResult.error) throw activeSubmissionResult.error;
    const activeInviteIds = new Set((activeSubmissionResult.data || []).map(item => String(item.invite_id || '')));
    const expiredInviteIds = (inviteResult.data || [])
      .filter(invite => tarotInviteCleanupDue(invite, activeInviteIds, now))
      .map(invite => invite.id);
    if (expiredInviteIds.length) {
      const inviteDelete = await supabase.from('tarot_invites').delete().in('id', expiredInviteIds);
      if (inviteDelete.error) throw inviteDelete.error;
    }
    return { deletedInvites:expiredInviteIds.length };
  }

  const submissionRows = await readJson(TAROT_SUBMISSIONS_FILE, []);
  const inviteRows = await readJson(TAROT_INVITES_FILE, []);
  const currentSubmissions = Array.isArray(submissionRows) ? submissionRows : [];
  const currentInvites = Array.isArray(inviteRows) ? inviteRows : [];
  const now = Date.now();
  ...
  const activeInviteIds = new Set(nextSubmissions.map(item => String(item.invite_id || item.inviteId || '')));
  const nextInvites = currentInvites.filter(invite => !tarotInviteCleanupDue(invite, activeInviteIds, now));
  if (nextSubmissions.length !== currentSubmissions.length) await writeJson(TAROT_SUBMISSIONS_FILE, nextSubmissions);
  if (nextInvites.length !== currentInvites.length) await writeJson(TAROT_INVITES_FILE, nextInvites);
  return {
    deletedSubmissions:currentSubmissions.length - nextSubmissions.length,
    deletedInvites:currentInvites.length - nextInvites.length
  };
}

async function deleteExpiredTarotSubmissions() {
  return deleteExpiredTarotData();
}
...
async function countTarotSubmissionsForInvite(inviteId) {
  ...
  const rows = await readJson(TAROT_SUBMISSIONS_FILE, []);
  return (Array.isArray(rows) ? rows : []).filter(item => String(item.inviteId || item.invite_id || '') === id).length;
}
...
function maxSubmissionsForInvite(invite) {
  ...
  return settings.singleUse === false || settings.single_use === false ? 999 : 1;
}

function assertDrawnCardsForInvite(invite, drawnCards, usedCount = 0) {
  if (!invite) {
...
async function submitTarotReadingWithTables(hash, payload, drawnCards) {
  ...
  const usedCount = invite ? await countTarotSubmissionsForInvite(invite.id) : 0;
  assertDrawnCardsForInvite(invite, drawnCards, usedCount);
  const now = Date.now();
  const submission = normalizeTarotSubmission({
    ...
    questionContext:normalizeTarotQuestionContext(payload.questionContext || payload.question_context),
    ...
    createdAt:now,
    updatedAt:now,
    deleteAt:now + TAROT_RETENTION_MS
  });
  let { data, error } = await supabase
    .from('tarot_submissions')
    ...
    .select('*')
    .single();
  if (error && isMissingColumnOrTable(error) && `${error.message || error.details || error.hint || ''}`.toLowerCase().includes('question_context')) {
    const fallbackSubmission = {
      ...submission,
      question:tarotQuestionContextFallback(submission.question, submission.questionContext),
      questionContext:{},
      question_context:{}
    };
    const fallback = await supabase
      .from('tarot_submissions')
      .insert(tarotSubmissionDbPayload(fallbackSubmission, { includeQuestionContext:false }))
      .select('*')
      .single();
    data = fallback.data;
  ...
  if (error) throw error;
  if (usedCount + 1 >= maxSubmissionsForInvite(invite)) {
...
async function submitTarotReading(payload) {
  ...
  if (supabase) return submitTarotReadingWithTables(hash, payload, drawnCards);
...
module.exports = {
  ...
  countTarotSubmissionsForInvite,
  updateTarotSubmission,
  deleteTarotSubmission,
  deleteExpiredTarotData,
