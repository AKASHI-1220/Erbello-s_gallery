...
function normalizeGalleryImages(value) {
  ...
  if (typeof value === 'string' && value.trim()) {
    try { const parsed = JSON.parse(value); return Array.isArray(parsed) ? parsed.filter(Boolean).slice(0, 8) : []; } catch (_) { return []; }
  }
  return [];
}
...
function maskPrivateArtifact(item) {
  ...
  if (!row.is_private) {
    const { private_password_hash, private_password_salt, ...safe } = row;
    return safe;
  }
  return {
    id: row.id,
...
async function listArtifacts(options = {}) {
  ...
  if (supabase) {
    let { data, error } = await supabase
      .from('artifacts')
      .select('id, title, description, type, tags, source_kind, is_jsx, created_at, updated_at, view_count, cover_image, gallery_images, detail_text, is_private')
      .order('updated_at', { ascending: false });

    if (error && isMissingColumnOrTable(error)) {
      const fallback = await supabase
        .from('artifacts')
        .select('id, title, description, type, tags, is_jsx, created_at, updated_at, view_count, is_private')
        .order('updated_at', { ascending: false });
      data = fallback.data;
    ...
    return (data || []).map(item => includePrivateDetails ? normalizeArtifact(item) : maskPrivateArtifact(item));
  }

  const items = await readJson(DATA_FILE, []);
  return (Array.isArray(items) ? items : [])
    .map(({ code, private_password_hash, private_password_salt, ...rest }) => includePrivateDetails ? normalizeArtifact(rest) : maskPrivateArtifact(rest))
    .sort(byCreatedDesc);
}
...
async function updateArtifact(id, payload) {
  ...
  if (supabase) {
    ...
    if (error && isMissingColumnOrTable(error)) {
      const { source_kind, cover_image, gallery_images, detail_text, is_private, private_password_hash, private_password_salt, ...fallbackPatch } = patch;
      const fallback = await supabase
        .from('artifacts')
    ...
    if (error && isMissingColumnOrTable(error)) {
      const { tags, source_kind, cover_image, gallery_images, detail_text, is_private, private_password_hash, private_password_salt, ...fallbackPatch } = patch;
      const fallback = await supabase
        .from('artifacts')
        .update(fallbackPatch)
        .eq('id', id)
