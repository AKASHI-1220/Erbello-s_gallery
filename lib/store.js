...
async function listArtifacts() {
  if (supabase) {
    let { data, error } = await supabase
      .from('artifacts')
      .select('id, title, description, type, tags, source_kind, is_jsx, created_at, updated_at, view_count, cover_image, gallery_images, detail_text')
...
async function updateArtifact(id, payload) {
  ...
  if (supabase) {
    ...
    if (error && isMissingColumnOrTable(error)) {
      const { source_kind, cover_image, gallery_images, detail_text, ...fallbackPatch } = patch;
      const fallback = await supabase
        .from('artifacts')
    ...
    if (error && isMissingColumnOrTable(error)) {
      const { tags, source_kind, cover_image, gallery_images, detail_text, ...fallbackPatch } = patch;
      const fallback = await supabase
        .from('artifacts')
        .update(fallbackPatch)
        .eq('id', id)
